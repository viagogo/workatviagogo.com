$(document).ready(function () {
  ...
  var forked = 0;
  var repos = [];
  var org = new Organization('viagogo', []);

  loadRepositories = function(org, page) {
    page = page || 1;

    $.get('https://api.github.com/users/viagogo/repos?per_page=100&page=' + parseInt(page), function(data) {
      if (data.length > 0) {
        ...
        org.addReposToContainer($('.projects .gogoprojects'), org.regularRepos());

        $('.project-count').html(org.forkedCount());
        $.get('https://api.github.com/orgs/viagogo/members', function(data) {
  ...
  $.get('https://api.github.com/orgs/viagogo/members', function(data) {
    users = data.length
    $('.stats-users').html("We have " + users + " viagogoers contributing to open source projects");
  });
